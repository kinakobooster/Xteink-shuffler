...
| カードを3枚引く | **Confirm**（ボタン2） | ランダム3枚を三角形配置で表示（縦横比維持） |
| カードを1枚引く | **Left / Right**（ボタン3〜4） | ランダムに1枚表示 |
...
ZIP を SD カードにコピーするのではなく、**解凍した中身**をルートに置いてください。

```text
正: SDルート/aaa/cover.bmp
誤: SDルート/aaa-deck.zip
誤: SDルート/aaa/aaa/000.bmp  （フォルダが二重）
```

Mac で展開すると `__MACOSX` フォルダができます。**削除してください**（ないとそちらがデッキとして選ばれ「No cards in deck」になります）。
